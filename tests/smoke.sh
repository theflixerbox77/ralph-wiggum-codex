...
schema_has_minimal_required_fields() {
  local schema_file="$1"
...
expected_required = {"status", "evidence", "next_step"}
expected_optional = {"no_change_justification", "completion_promise"}
if required != expected_required:
    raise SystemExit(1)
if not expected_optional.issubset(properties):
    raise SystemExit(1)
PY
}
...
expect_success "dry-run config works" run_loop schema_complete_change --cwd "$base_repo" --prompt "x" --max-iterations 2 --idle-timeout-seconds 12 --hard-timeout-seconds 34 --timeout-retries 2 --progress-scope . --dry-run >/dev/null
...
expect_success "schema completion with change stops loop" run_loop schema_complete_change --cwd "$repo_schema" --state-dir "$state_schema" --prompt "ship it" --max-iterations 3 >/dev/null
...
expect_success "completion schema keeps optional compatibility fields optional" schema_has_minimal_required_fields "$state_schema/completion-schema.json"

repo_schema_minimal="$tmp_dir/repo-schema-minimal"
state_schema_minimal="$repo_schema_minimal/.codex/ralph-loop"
make_repo "$repo_schema_minimal"
expect_success "minimal schema completion with change stops loop" run_loop schema_complete_change_minimal --cwd "$repo_schema_minimal" --state-dir "$state_schema_minimal" --prompt "ship it" --max-iterations 2 >/dev/null
expect_success "minimal schema completion stop reason recorded" grep -q "schema_completion_detected" "$state_schema_minimal/run-summary.md"

repo_promise_mismatch="$tmp_dir/repo-promise-mismatch"
state_promise_mismatch="$repo_promise_mismatch/.codex/ralph-loop"
make_repo "$repo_promise_mismatch"
expect_success "missing completion promise does not satisfy compatibility mode" run_loop schema_complete_change_minimal --cwd "$repo_promise_mismatch" --state-dir "$state_promise_mismatch" --prompt "ship it" --completion-promise "DONE" --max-iterations 1 >/dev/null 2>&1
expect_success "completion promise mismatch event logged" grep -q "completion_promise_mismatch" "$state_promise_mismatch/events.log"
...
expect_success "timeout retry reaches completion" run_loop timeout_then_complete --cwd "$repo_timeout" --state-dir "$state_timeout" --prompt "work" --max-iterations 3 --idle-timeout-seconds 1 --hard-timeout-seconds 2 --timeout-retries 1 >/dev/null 2>&1
...
if run_loop always_timeout --cwd "$repo_timeout_output" --state-dir "$state_timeout_output" --prompt "work" --max-iterations 1 --idle-timeout-seconds 1 --hard-timeout-seconds 2 --timeout-retries 0 >"$timeout_output_log" 2>&1; then
  pass "timeout scenario for output-noise check completes"
else
  fail "timeout scenario for output-noise check completes"
fi
if rg -q "Terminated: 15" "$timeout_output_log"; then
  fail "timeout scenario omits bash terminated noise"
else
  pass "timeout scenario omits bash terminated noise"
fi
...
make_repo "$repo_noop"
expect_success "no-op without justification does not complete" run_loop schema_complete_nochange --cwd "$repo_noop" --state-dir "$state_noop" --prompt "work" --max-iterations 1 --progress-scope . >/dev/null
...
expect_success "no-op with justification can complete" run_loop schema_complete_nochange_justified --cwd "$repo_noop_ok" --state-dir "$state_noop_ok" --prompt "work" --max-iterations 2 --progress-scope . >/dev/null
...
CWD=/tmp/old
EOF_LOCK
expect_success "stale lock with dead pid auto-reclaims" run_loop schema_in_progress --cwd "$repo_lock_ok" --state-dir "$state_lock_ok" --prompt "x" --dry-run >/dev/null 2>&1
...
expect_success "malformed lock metadata can be reclaimed explicitly" run_loop schema_in_progress --cwd "$repo_lock_bad" --state-dir "$state_lock_bad" --prompt "x" --reclaim-stale-lock --dry-run >/dev/null 2>&1

repo_resume="$tmp_dir/repo-resume"
state_resume="$repo_resume/.codex/ralph-loop"
make_repo "$repo_resume"
expect_success "initial timeout run leaves resumable state" run_loop always_timeout --cwd "$repo_resume" --state-dir "$state_resume" --prompt "recover" --max-iterations 1 --idle-timeout-seconds 1 --hard-timeout-seconds 2 --timeout-retries 0 >/dev/null 2>&1
